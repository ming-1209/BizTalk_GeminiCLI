...
- [x] 프로트엔드 `index.html` 페이지가 FastAPI 서버에서 요청 되도록 정적라우팅(static routing) 기능이 있어야 한다
...
- [ ] 백엔드와 프로트엔드 모두 Vercel에 같이 배포(Deploy) 되도록 한다
...
5. [x] `requirements.txt` 에 라이브러리의 버전을 명시하고, 최신버전을 사용한다
6. [x] `requirements.txt` 작성 및 가상환경 폴더를 생성한 이후에 패키지 설치
